...
test('river shoreline accent is layered as a visible ring between outer and inner banks',()=>{
  ...
  const outer=block.indexOf("c.strokeStyle='#5e432b'")
  const shore=block.indexOf("c.strokeStyle='#a77a47'")
  const inner=block.indexOf("c.strokeStyle='#16396e'")
...
test('river water uses subdued highlight rather than a glossy center stripe',()=>{
  const source=read('src/game/world-art.js')
  assert.match(source,/waterWidth:17/)
  assert.match(source,/highlightWidth:1\.35/)
