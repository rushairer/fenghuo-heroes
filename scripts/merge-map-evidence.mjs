...
const output=separator>=0?args[separator+1]:'map-evidence.merged.json'

if(inputs.length<2){
  console.error('usage: node scripts/merge-map-evidence.mjs <a.json> <b.json> [more.json ...] [--out output.json]')
  process.exitCode=2
}else if(!output){
  ...
  try{
    ...
    const merged=mergeCanonicalEvidenceBundles(...bundles)
    const outputPath=resolve(output)
    writeFileSync(outputPath,JSON.stringify(merged,null,2)+'\n','utf8')
    console.log(`merged ${inputs.length} evidence bundles: ${outputPath}`)
  }catch(error){
