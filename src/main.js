...
class App{
  constructor(canvas){this.canvas=canvas;this.r=makeRenderer(canvas);this.hd=true;this.r.syncResolution({pixelPreview:!this.hd});this.store=new GameStore(window.localStorage);this.audio=new AudioBus();this.input=new Input(window);this.assets=new AssetRegistry();this.assetsReady=this.assets.load();this.scene=null;this.last=performance.now();this.playerCount=1;this.frame=this.frame.bind(this);const forced=new URLSearchParams(location.search).get('scene');if(forced==='players')this.go('players');else if(forced==='setup')this.go('setup');else if(forced==='strategy'){this.store.newGame({scenarioYear:189,humanFactions:['liu']});this.go('strategy')}else this.go('title');requestAnimationFrame(this.frame);canvas.addEventListener('pointerdown',()=>canvas.focus());window.addEventListener('resize',()=>this.syncRenderResolution(),{passive:true});canvas.focus()}
  ...
  syncRenderResolution(){this.r.syncResolution({pixelPreview:!this.hd})}
  toggleHd(){this.hd=!this.hd;this.canvas.classList.toggle('pixel-preview',!this.hd);this.syncRenderResolution();this.audio.move()}
