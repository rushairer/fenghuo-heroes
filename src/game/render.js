...
export function makeRenderer(canvas) {
  ...
  function portraitBust(cx,baseY,scale,tone,flip=false,kind=0){ctx.save();ctx.translate(X(cx),Y(baseY));ctx.scale(flip?-1:1,1);const s=S*scale;ctx.fillStyle=tone;ctx.strokeStyle='#1b0a07';ctx.lineWidth=Math.max(2,s*.7);ctx.beginPath();ctx.ellipse(0,-30*s/S,17*s/S,22*s/S,0,0,Math.PI*2);ctx.fill();ctx.stroke();ctx.fillStyle='#22130c';ctx.beginPath();ctx.moveTo(-17*s/S,-42*s/S);ctx.quadraticCurveTo(0,-58*s/S,19*s/S,-40*s/S);ctx.lineTo(15*s/S,-34*s/S);ctx.quadraticCurveTo(0,-45*s/S,-16*s/S,-34*s/S);ctx.closePath();ctx.fill();if(kind%2===0){ctx.fillStyle='#23120d';ctx.beginPath();ctx.moveTo(-10*s/S,-15*s/S);ctx.quadraticCurveTo(0,8*s/S,11*s/S,-15*s/S);ctx.quadraticCurveTo(7*s/S,18*s/S,0,28*s/S);ctx.quadraticCurveTo(-7*s/S,-15*s/S,-10*s/S,-15*s/S);ctx.fill()}ctx.fillStyle=kind%3===0?'#5f1914':'#2f2730';ctx.beginPath();ctx.moveTo(-24*s/S,-7*s/S);ctx.lineTo(24*s/S,-7*s/S);ctx.lineTo(34*s/S,37*s/S);ctx.lineTo(-34*s/S,37*s/S);ctx.closePath();ctx.fill();ctx.restore()}
