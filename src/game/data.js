...
import { activeMapProfile } from './map-activation.js'
...
export const FACTION_BY_ID=Object.freeze(
  Object.fromEntries(FACTIONS.map((faction)=>[faction.id,faction])),
)

// Runtime map activation has two gates: source-backed evidence readiness and an
// explicit activation target. The active target is still scaffold, so this is a
// behavior-preserving refactor that makes the future canonical switch auditable.
export const MAP_PROFILE=activeMapProfile()
export const CITIES=MAP_PROFILE.cities
export const CITY_BY_ID=MAP_PROFILE.cityById

export const CATEGORY_LABELS=Object.freeze(
  Object.fromEntries(INSPECTION_CATEGORY_SCHEMA.map(({id,label})=>[id,label])),
)
