...
export class StrategyScene extends BaseStrategyScene{
  ...
  drawMarchCompose(){const r=this.app.r,city=this.app.store.mapProfile.cityById[this.marchFrom],daily=dailyFoodFor(this.marchTroops,1);r.panel(72,42,176,126,'#000','#9b6514');r.text(`${city?.name??''} 出陣`,160,51,11,'#efd27d','center','top',SERIF,'700');const rows=[['兵力',this.marchTroops],['軍資金',this.marchGold],['兵糧',this.marchFood],['路線','指定']];rows.forEach(([label,value],i)=>{r.text(`${i===this.composeFocus?'▶':'　'}${label}`,99,76+i*19,8,i===this.composeFocus?COLORS.cyan:'#ddd0ad');r.text(value,218,76+i*19,8,i===this.composeFocus?COLORS.cyan:'#eee0bd','right')});r.text('武將 1（名冊待校準）',160,145,6,'#8e846f','center');r.text(`1日兵糧 = 兵${this.marchTroops}÷100 + 武將1 = ${daily}`,160,156,6,'#a99d82','center')}
