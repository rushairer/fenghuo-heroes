...
import { inspectionCommandItems, inspectionCommandPath } from '../game/inspection-command-parity.js'
...
export class StrategyScene{
  constructor(app){this.app=app;if(!app.store.hasGame()&&!app.store.load()){app.go('title');return}this.view='map';this.stage=app.store.mode==='inspection'?'survey':'march';this.menuIndex=0;this.category=app.store.inspectionCategoryForActive()??'domestic';this.message='';this.targetCity=null;this.infoTab=0;this.marchFrom=null;this.marchTargets=[];this.marchIndex=0;this.saveIndex=0;this.commandSubmenu=null;this.snapCursorToOwnedCity()}
  ...
  updateMap(b){const store=this.app.store,s=store.state;if(b==='LEFT')this.moveCursor(-1,0);if(b==='RIGHT')this.moveCursor(1,0);if(b==='UP')this.moveCursor(0,-1);if(b==='DOWN')this.moveCursor(0,1);if(b==='B'){this.app.audio.cancel();this.app.go('title');return}if(this.stage==='survey'){if(b==='A'){this.infoTab=0;this.view='info';this.app.audio.confirm();return}if(b==='START'){this.infoTab=1;this.view='info';this.app.audio.confirm();return}if(b==='C'){const city=store.cityAt(s.cursor.x,s.cursor.y,12);if(city){this.targetCity=city.id;this.view='city-status';this.app.audio.confirm()}else{this.stage='command';this.openCategory()}}return}if(this.stage==='command'){if(b==='A'){this.infoTab=0;this.view='info';this.app.audio.confirm();return}if(b==='START'){this.requestFinishTurn();return}if(b==='C'){const city=store.cityAt(s.cursor.x,s.cursor.y,12),locked=store.inspectionCategoryForActive();if(!city){this.openCategory()}else if(!locked){this.message='先把方框移到地图空白处按 C，决定本月是内政、外交还是军备。';this.view='message';this.app.audio.alert()}else if(s.cities[city.id].owner!==store.humanFaction){this.message='只能向本国城池下令。';this.view='message';this.app.audio.alert()}else{this.category=locked;this.targetCity=city.id;this.view='commands';this.menuIndex=0;this.app.audio.confirm()}}return}if(this.stage==='march'){if(b==='A'){this.infoTab=1;this.view='info';this.app.audio.confirm();return}if(b==='START'){this.requestFinishTurn();return}if(b==='C'){const city=store.cityAt(s.cursor.x,s.cursor.y,12);if(!city||s.cities[city.id].owner!==store.humanFaction){this.message='行军时请把方框移到本国城池后按 C。';this.view='message';this.app.audio.alert()}else{this.marchFrom=city.id;this.marchTargets=[...city.neighbors];this.marchIndex=0;this.view='march-dest';this.app.audio.confirm()}}}}
  ...
  resetForActiveTurn(){this.view='map';this.stage=this.app.store.mode==='inspection'?'survey':'march';this.category=this.app.store.inspectionCategoryForActive()??'domestic';this.menuIndex=0;this.commandSubmenu=null;this.targetCity=null;this.marchFrom=null;this.marchTargets=[];this.marchIndex=0;this.snapCursorToOwnedCity()}
