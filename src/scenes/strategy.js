...
import { CATEGORY_LABELS, CITIES, FACTION_BY_ID } from '../game/data.js'
...
export class StrategyScene{
  constructor(app){this.app=app;if(!app.store.hasGame()&&!app.store.load()){app.go('title');return}this.roadPairs=uniqueRoadPairs(this.mapCities());this.view='map';this.stage=app.store.mode==='inspection'?'survey':'march';this.menuIndex=0;this.category=app.store.inspectionCategoryForActive()??'domestic';this.message='';this.targetCity=null;this.infoTab=0;this.marchFrom=null;this.saveIndex=0;this.commandSubmenu=[];this.infoReturnView='map';this.infoCommandBrowse=false;this.taxRateDraft=0;this.taxRateOriginal=null;this.transportSource=null;this.transportDestination=null;this.transportHint='';this.transportLoadIndex=0;this.snapCursorToOwnedCity()}
  ...
  mapCities(){return this.app.store.mapProfile?.cities??CITIES}
  ...
  drawRoads(camera){
    const segments=this.roadPairs.map(([from,to])=>({
      a:toScreen(cityWorldPoint(this.cityById(from)),camera),
      b:toScreen(cityWorldPoint(this.cityById(to)),camera),
    })).filter(({a,b})=>!(
      (a.x<-24&&b.x<-24)||
    ...
    drawRoadNetwork(this.app.r,segments)
  }
