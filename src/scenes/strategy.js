...
const cursorStep=8
const ROAD_PAIRS=uniqueRoadPairs(CITIES)
export class StrategyScene{
  ...
  drawRoads(camera){
    const segments=ROAD_PAIRS.map(([from,to])=>({
      a:toScreen(cityWorldPoint(CITY_BY_ID[from]),camera),
      b:toScreen(cityWorldPoint(CITY_BY_ID[to]),camera),
    })).filter(({a,b})=>!(
      (a.x<-24&&b.x<-24)||
